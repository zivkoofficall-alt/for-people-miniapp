...
import { X, User, Check, CreditCard, Sparkles, TrendingUp } from "lucide-react";
import { styles } from "../theme.js";
import { PLAN_FEATURES, PRO_PRICE_LABEL } from "../constants.js";
import { computeContactStats } from "../helpers.js";

export default function Profile({ subscription, contacts, tasks, onClose, onPayWithStars, onDowngradeToFree }) {
  ...
  async function handlePayClick() {
    ...
    setPayLoading(true);
    try {
      await onPayWithStars();
    } catch (e) {
      setPayError(e.message || "Не получилось открыть оплату");
    } finally {
      setPayLoading(false);
    }
  }
        ...
        <div style={{ ...styles.planCard, ...styles.planCardPro }}>
          ...
          {!isPro && (
            <>
              <button
                className="fp-btn"
                style={{ ...styles.primaryPill, width: "100%", marginTop: 8, opacity: payLoading ? 0.7 : 1 }}
                onClick={handlePayClick}
                disabled={payLoading}
              >
                <CreditCard size={14} /> {payLoading ? "Открываем оплату…" : `Оплатить ${PRO_PRICE_LABEL.replace("/мес", "")} звёздами`}
              </button>

              {payError && (
                <div style={styles.payDisclaimer}>{payError}</div>
              )}
            </>
          )}
        </div>
