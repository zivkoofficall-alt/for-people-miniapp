...
export const storage = {
  async get(key, _shared) {
    ...
    const localTs = parseInt(localGetSync(`${k}__ts`) || "0", 10) || 0;
    ...
    try {
      // Метку времени сравниваем ДО того, как тянуть весь (потенциально
      // чанкованный) value — так дешевле и, что важнее, отражает, действительно
      // ли в облаке лежит более свежая версия, а не просто "хоть что-то".
      // Метка в облаке проставляется только после успешной записи ВСЕХ чанков
      // (см. set ниже), поэтому оборванная на середине запись в облако не
      // выглядит "новее" локальной копии и не может её затереть.
      const cloudTsRaw = await tgGetItem(cs, `${k}__ts`);
      const cloudTs = parseInt(cloudTsRaw || "0", 10) || 0;

      if (cloudTs > localTs) {
        const cloudRaw = await chunkedGet((kk) => tgGetItem(cs, kk), k);
        if (cloudRaw) {
          // Облако реально новее локальной копии — подтягиваем localStorage к нему.
          localSetSync(k, cloudRaw);
          localSetSync(`${k}__ts`, String(cloudTs));
    ...
    // Облако либо не новее локальной копии, либо пусто/недоступно/упало —
    // не теряем данные, отдаём то, что реально лежит в localStorage.
    return localRaw ? { key, value: localRaw } : null;
  },
  ...
  async set(key, value, _shared) {
    ...
    // Метка времени пишется тут же — локальная копия всегда "самая свежая".
    const localOk = localSetSync(k, value);
    localSetSync(`${k}__ts`, String(ts));

    // Шаг 2: асинхронная запись в CloudStorage — для синхронизации между устройствами.
    // Метку времени в облаке проставляем ТОЛЬКО после того, как все чанки
    // реально долетели — если приложение закроют посреди записи, облачная
    // метка останется старой и не сможет на следующем запуске затереть более
    // свежую localStorage-копию (см. get выше).
    const cs = getTelegramCloudStorage();
    if (cs) {
      chunkedSet((kk, v) => tgSetItem(cs, kk, v), k, value)
        .then(() => tgSetItem(cs, `${k}__ts`, String(ts)))
        .catch((e) => {
