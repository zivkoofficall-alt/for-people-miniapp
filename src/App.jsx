...
import { emptyMessengers, emptyContact, emptyTask, emptyGoal, emptySubscription, computeGoalProgress, buildContactLink, initials, pad, formatRuPhone, pluralPeople, csvEscape, resizeImageFile, nextRepeatDate, buildReminderAt } from "./helpers.js";
...
export default function ForPeople() {
  ...
  async function exportCsv() {
    ...
    // Если открыты внутри Telegram — пробуем отправить файл документом в чат
    // через бэкенд (см. api/send-csv.js). У Mini App нет способа сделать это
    // без backend, поэтому при любой проблеме — тихо откатываемся на обычное
    // скачивание в браузере, чтобы человек в любом случае получил файл.
    ...
    if (tg && tg.initData && sendCsvUrl) {
      try {
        ...
        console.warn("send-csv failed, falling back to browser download", data);
      } catch (e) {
        console.warn("send-csv request failed, falling back to browser download", e);
      }
    }
  ...
  // Реальная оплата через Telegram Stars (см. api/create-invoice.js и
  // api/telegram-webhook.js). Сервер только создаёт ссылку на счёт — сам
  // факт оплаты подтверждает Telegram.WebApp.openInvoice() своим callback'ом,
  // это официальный способ, ему можно доверять без дополнительной проверки
  // с бэкенда.
  async function handlePayWithStars() {
    const tg = window.Telegram && window.Telegram.WebApp;
    const createInvoiceUrl = import.meta.env.VITE_CREATE_INVOICE_URL;

    if (!tg || !tg.initData) {
      throw new Error("Оплата доступна только внутри Telegram");
    }
    if (!createInvoiceUrl) {
      throw new Error("Оплата не настроена: не задан VITE_CREATE_INVOICE_URL в .env");
    }
    ...
    const data = await response.json();
    if (!response.ok || !data.ok) {
      throw new Error(data.error || "Не получилось создать счёт на оплату");
    }

    // openInvoice сам открывает нативное окно оплаты Stars и вызывает callback
    // со статусом, когда пользователь закрывает окно: "paid" | "cancelled" | "failed".
        ...
        <nav style={styles.bottomBar}>
          ...
          <button className="fp-btn" style={styles.bottomBarItem} onClick={() => setTaskBoardOpen(true)}>
            ...
            {tasks.filter((t) => t.status !== "done").length > 0 && (
              <span style={{ ...styles.iconBtnBadge, top: -2, right: 6 }}>{tasks.filter((t) => t.status !== "done").length}</span>
            )}
