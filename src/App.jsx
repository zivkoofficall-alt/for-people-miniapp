...
import { emptyMessengers, emptyContact, emptyTask, emptyGoal, emptySubscription, computeGoalProgress, buildContactLink, initials, pad, formatRuPhone, pluralPeople, csvEscape, resizeImageFile } from "./helpers.js";
...
export default function ForPeople() {
  ...
  // dueDate/contactId/important). completedAt проставляется/сбрасывается
  // автоматически при переходе в 'done' и обратно — пригодится для сортировки
  // вкладки "Готово" по свежести выполнения.
  async function handleUpdateTask(taskId, patch) {
    const next = tasks.map((t) => {
      if (t.id !== taskId) return t;
      const merged = { ...t, ...patch };
      if (patch.status) merged.completedAt = patch.status === "done" ? Date.now() : null;
      return merged;
    });
