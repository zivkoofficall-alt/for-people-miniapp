...
import { describeScreenshotBug, transcribeVoice } from "./_lib/gemini.js";
...
async function handleBugBotUpdate(update, botToken) {
  ...
  if (text === "/diag") {
    ...
    await sendMessage(botToken, chatId, ["Переменные окружения:", ...lines, "", chatStatus, supabaseReachable].join("\n"));
    return true;
  }
  ...
  if (text === "/endpoint") {
    ...
    const prompt = buildEndpointPrompt(rows, { version: `v${newVersion}`, dateStr });
