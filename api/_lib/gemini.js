 ...
 * captionText — подпись, которую пользователь мог прикрепить к фото в
 * Telegram (например "кнопка не нажимается"). Раньше подпись вычислялась
 * в telegram-webhook.js, но никуда не передавалась, и Gemini описывал
 * только то, что видно на картинке, полностью игнорируя текст пользователя
 * — теперь оба источника анализируются вместе, и подпись не теряется.
 */
export async function describeScreenshotBug(base64, mimeType, captionText) {
  const caption = (captionText || "").trim();
  const instruction = caption
    ? `Пользователь прислал скриншот бага с подписью: "${caption}". Опиши коротко на русском итоговую суть проблемы, объединив то, что написано в подписи, и то, что видно на самом скриншоте — учитывай оба источника, не игнорируй подпись. 1-3 предложения по делу, без вступлений и без markdown-разметки.`
    : "Опиши коротко на русском, какая проблема/баг виден на этом скриншоте интерфейса приложения. 1-3 предложения по делу, без вступлений и без markdown-разметки.";
  return callGemini([
    { text: instruction },
